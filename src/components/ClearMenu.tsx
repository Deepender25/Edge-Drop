...
interface ClearMenuProps {
  /** Full unfiltered item list (pinned + recent) to compute time-window ids from. */
  items: ClipboardItemDto[]
  disabled: boolean
  /** Panel's own open/closed state — closes this menu whenever the panel closes. */
  panelOpen: boolean
  /** Clear a specific set of ids (used for the time-window options). */
  onClear: (ids: string[]) => void
  /** Clear all unpinned history (mirrors the existing no-arg clear() call). */
  onClearAll: () => void
}
...
//export function ClearMenu({ items, disabled, onClear, onClearAll }: ClearMenuProps) {
export function ClearMenu({ items, disabled, panelOpen, onClear, onClearAll }: ClearMenuProps) {
  ...
  // The panel itself can close (edge-leave, Alt+C, click-away) without this
  // menu's own click-outside handler ever firing — e.g. the whole blade
  // sliding off-screen while the cursor stays put. Force-close in that case
  // so it isn't sitting open when the panel next slides back out.
  useEffect(() => {
    if (!panelOpen) {
      setOpen(false)
      setConfirmAll(false)
  ...
  // Re-arm the "Clear all" confirmation every time the menu closes, so it
  // can never be actioned by a stray click on a later, unrelated open.
  useEffect(() => {
  ...
  const clearWindow = (hours: number) => {
    ...
    // pinned items are never included in a bulk clear, regardless of window
